...
$Sellcoins = $ObjSellcoins->GetBitCoinForPasskey($passkey);
...
if(!empty($Sellcoins)){
	...
	$SellcoinsRow = $SellcoinsRow[0];

    if($SellcoinsRow["seller_aprovel"] == 1){
    	echo'<pre>';print_r("Seller Already approved"); die;
    } 

	$add_date_time = strtotime($SellcoinsRow["add_date"]);
	...
	if ($timeDiferenceInMinute <= 10) {
		$ObjSellcoins->ApproveSellerForPasskey($passkey);
