<?php
	class Buycoins extends dbClass{
		...
		public function Buycoinsadd($UserId,$amount,$user_wallet,$seller_userid){
	  	 	$this->sql = "INSERT INTO ".$this->tables['buy_bit_coin']." (buyer_user_id , amount , user_wallet , seller_userid , add_date)                          value ('".$this->CleanData($UserId)."' , '".$this->CleanData($amount)."' , 
			              '".$this->CleanData($user_wallet)."' , '".$this->CleanData($seller_userid)."' , '".date('Y-m-d H:i:s')."')"; 
		...
		public function GetBuyCoinRequestStatus($Tid){
		 	$this->sql = "Select seller_aprovel from ".$this->tables['buy_bit_coin']." WHERE id = '".$this->CleanData($Tid)."' ";
	  		return $this->query($this->sql,1);
		}

		public function ApproveSellerForPasskey($TransactionId){
		   	$sql = "Update ".$this->tables['buy_bit_coin']." set seller_aprovel = '1' Where id = '".$TransactionId."'"; 
            $this->query($sql,0);
		}
	    ...
	    public function ValidAmount($UserId){
		    $this->sql = "Select Cash_wallet from user where id='".$this->CleanData($UserId)."' ";
	  		return $this->query($this->sql,1);
		}
		...
		public function GetBuyCoinTransactionDetail($TransactionId){
			$this->sql = "Select * from ".$this->tables['buy_bit_coin']." where id = '".$this->CleanData($TransactionId)."' ";
	  		return $this->query($this->sql,1);
		}
