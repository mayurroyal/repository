...
//POPUP
var lefta=($(window).width()-$('.popup-one').width())/2,
      topa=($(window).height()-$('.popup-one').height())/2;

$('.ph-uplod').click(function(){
	$('.popup-one').css({left:lefta,top:topa}).fadeIn();
	$('.overlay').fadeIn();
	return false;
	});
	$('.popup-close').click(function(){
	$('.popup-one, .overlay').fadeOut();
});

$(window).resize(function() {
	lefta=($(window).width()-$('.popup-one').width())/2,
      topa=($(window).height()-$('.popup-one').height())/2;

      $('.popup-one').css({left:lefta,top:topa});
});	    
