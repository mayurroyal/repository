<?php
	class Buycoins extends dbClass{
		...
		public function UpdateSearchTrustedUserSetting($setting_name, $setting_value, $UserId){
			$sql = "Update ".$this->tables['other_setting']." set 'setting_name' =  '".$this->CleanData($setting_name)."' , 'setting_value' =  '".$this->CleanData($setting_value)."' Where buyer_userid = '".$UserId."'";
            $this->query($sql,0);
		}
		...
		public function SelectSearchTrustedUserSetting($UserId){
      	...
      	public function ByuerSettingValueValidation($setting_value){
		   $this->sql = "Select * from ".$this->tables['other_setting']." where setting_value='".$this->CleanData($setting_value)."'";
		   return $this->query($this->sql,1);

		}

       public function ByuerSettingNameValidation($setting_name){
		   $this->sql = "Select * from ".$this->tables['other_setting']." WHERE setting_name='".$this->CleanData($setting_name)."'";
		   return $this->query($this->sql,1);
		}
