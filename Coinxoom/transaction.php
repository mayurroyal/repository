	...
	if(isset($_POST['buycoin']) && $_POST['buycoin'] != ''){
       //echo"<pre>";print_r($_POST); die;
        ...
        $sendMail = 1;
			...
			$ArrRes = $ObjBuycoins->GetEscrowTransactionDetail($TransactionId);
				
				
				$CashWallet = $ArrRes[0]['cashwallet'];
				$CoinWallet = $ArrRes[0]['coinwallet'];
				
				$ObjBuycoins->UpdateCoinWalletSeller($seller_userid , $CoinWallet);
						  $ObjBuycoins->UpdateCashWalletbuyer($UserId , $CashWallet);
...
$ArrRes = $ObjBuycoins->GetEscrowTransactionDetail($tid);
	//var_dump($tid);
				...
				$CashWallet = $ArrRes[0]['cashwallet'];
				$CoinWallet = $ArrRes[0]['coinwallet'];
				$ObjBuycoins->UpdateCancelCoinWallet($UserId , $CashWallet);
				$ObjBuycoins->UpdateCancelCashWallet($sid , $CoinWallet);
				}
#######################################  Code for buy coin end ##############################################


#######################################  Code for sell bit coin start ##############################################
   
	
	if(isset($_POST['sellcoin']) && $_POST['sellcoin'] != ''){
      $ip             = $_SERVER['REMOTE_ADDR'];
		$amount     	= $_POST['amount'];
		$user_wallet    = $_POST['user_wallet'];
		...
		//echo'<pre>';print_r($passkey); die;
		  if($_SESSION['Coin_wallet'] < $amount){
            echo "insufficient amount"; die;
        }
        ...
        $TransactionId  = 	$ObjSellcoins->Sellcoinsadd($UserId,$amount,$user_wallet,$buyer_userid);
...
<div class="transition-timer">
    ...
    <input type="hidden" name="hid_tid" value="<?php echo $TransactionId; ?>">
    <input type="hidden" name="hid_s_uid" value="<?php echo $seller_userid; ?>">
...
function checksellcoinCurrentStatus () {
		var tid = $("#buy_transaction_id").val();
		//alert("buy_coin_request_status.php?transactionid="+tid);
		var message = '';
		...
		$.ajax({
			type: "post",
			data : {transactionid : tid},
			url: "sell_coin_request_status.php",
			success: function(returnData) {
				if(returnData == "invalid_transaction"){
					message = "Some Error Occered, Please Try Again";
					killTimer();
	...
	function UpdatesellCoinTransaction(tid){
        //alert("update_sell_coin_request.php?transactionid="+tid);
		$.ajax({
			type: "post",
			data : {transactionid : tid},
			url: "update_sell_coin_request.php",
			success: function(returnData) {
				if(returnData == "error"){
					message = "There is some error. Please try again";
					$('#countdown').html(message);
...
if(isset($_POST['sellcoin']) && $_POST['sellcoin'] != ''){ ?>
	...
	$(document).ready(function(){
		if($('#countdown')){
			seconds = 600;
			countdownTimer = setInterval(checksellcoinCurrentStatus, 10*1000);	
			timer = setInterval(showTime, 1*1000);
		}		
	});
